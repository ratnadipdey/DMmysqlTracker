...
ALTER TABLE x_workorder
ADD COLUMN `ORIGINATORCODE` VARCHAR(50) ,
ADD COLUMN `RECEIVERCODE` VARCHAR(50) ;
																
CREATE DEFINER="root"@"%" TRIGGER `dmsi`.`activityonworkorder_AFTER_INSERT` AFTER INSERT ON `activityonworkorder` FOR EACH ROW
BEGIN
DECLARE v_workorderid VARCHAR(4000)                 /* Use -meta option workorder.WORKORDERID%TYPE */;
DECLARE v_event VARCHAR(4000)                         /* Use -meta option activityevents.EVENTID%TYPE */;
DECLARE v_version                       SMALLINT;
DECLARE e_msg                           VARCHAR(400);
DECLARE step                            VARCHAR(30);
DECLARE v_receiverid VARCHAR(4000);
DECLARE v_receivercode VARCHAR(4000);
DECLARE temp1 varchar(4000);
DECLARE act_count double;
DECLARE WO_CREATED tinyint default 1;
declare WO_ASSIGNED               tinyint default  2;
...
if v_event = WO_ASSIGNED THEN
select receiverid into v_receiverid FROM  WORKORDER  WHERE WORKORDERID = v_workorderid ;
select receivercode into v_receivercode FROM  WORKORDER  WHERE WORKORDERID = v_workorderid ;
UPDATE X_WORKORDER
SET
RECEIVERID = v_receiverid,
RECEIVERCODE=v_receivercode
WHERE
workorderid = v_workorderid;
end if;
												...
												CREATE DEFINER="root"@"%" TRIGGER `dmsi`.`activityonworkorder_AFTER_UPDATE` AFTER UPDATE ON `activityonworkorder` FOR EACH ROW
BEGIN

     DECLARE    v_workorderid                   BIGINT;
     DECLARE    v_event                       BIGINT;
     DECLARE    v_version                       BIGINT;
     DECLARE    e_msg                          VARCHAR(400);
     DECLARE    step                             VARCHAR(30);
     DECLARE    v_receiverid                    BIGINT;
	 DECLARE    v_receivercode                   VARCHAR(400);
  DECLARE    temp1 varchar(4000);
DECLARE    act_count BIGINT;
     
    DECLARE WO_CREATED     BIGINT    DEFAULT 1;
    DECLARE WO_ASSIGNED    BIGINT    DEFAULT 2;
    DECLARE WO_ACCEPTED    BIGINT    DEFAULT 3;
    DECLARE WO_REJECTED    BIGINT    DEFAULT 4;
    DECLARE WO_ACTIVE      BIGINT    DEFAULT 5;
    DECLARE WO_COMPLETED   BIGINT    DEFAULT 6;
    DECLARE WO_AMENDED     BIGINT    DEFAULT 7;
    DECLARE WO_AMENDED_APROVED   BIGINT       DEFAULT 8;
    DECLARE WO_AMENDED_DCLINE    BIGINT    DEFAULT 9;
    DECLARE WO_CANCELED          BIGINT    DEFAULT  10;
    DECLARE WO_CANCELED_AKNWLG   BIGINT    DEFAULT 11;
    DECLARE WO_REASSIGNED        BIGINT    DEFAULT 12;
    DECLARE WO_UPDATED           BIGINT    DEFAULT 13;
    DECLARE WO_ROLLBACK_ACCEPT   BIGINT    DEFAULT 19;
    DECLARE WO_UNCOMMIT_EMPTY_RETURN   BIGINT DEFAULT 20;
...
SET step := 'X_EQUIPMENTONWORKORDER';
     ...
     EQUIPMENTID,NETWEIGHT,TAREWEIGHT,GROSSWEIGHT,WEIGHTUNITS,SHIPMENTNUMBER,FREIGHTDESCRIPTION,
     SEAL,EMPTYRELEASENUMBER,HAZMAT,OVERWEIGHT,STATUSID,VOLUME,PIECECOUNT,COMMENTS,EQUIPMENTNUMBER,
     WEIGHTUOM,VOLUMEUOM, VERSION,LASTMODIFIEDBY,LASTMODIFIED, HEIGHT, WIDTH, LENGTH, DIMENSIONUOM, CHASSISNUMBER)
         select EQUIPMENTONWORKORDERID,WORKORDERID,SHIPPINGID,EQUIPMENTTYPECODE,EQUIPMENTID,
         NETWEIGHT,TAREWEIGHT,GROSSWEIGHT,WEIGHTUNITS,SHIPMENTNUMBER,FREIGHTDESCRIPTION,SEAL,
         EMPTYRELEASENUMBER,HAZMAT,OVERWEIGHT,STATUSID,VOLUME,PIECECOUNT,COMMENTS,EQUIPMENTNUMBER,
         WEIGHTUOM,VOLUMEUOM, v_version,LASTMODIFIEDBY,SYSDATE(), HEIGHT, WIDTH, LENGTH, DIMENSIONUOM, CHASSISNUMBER
           from EQUIPMENTONWORKORDER
          where WORKORDERID = v_workorderid;
...
if v_event = '2' THEN
     select receiverid into v_receiverid FROM  WORKORDER  WHERE WORKORDERID = v_workorderid ;
	 select receivercode into v_receivercode FROM  WORKORDER  WHERE WORKORDERID = v_workorderid ;
     UPDATE X_WORKORDER SET RECEIVERID = v_receiverid WHERE workorderid = v_workorderid ;
